<div id="box">
	<form action="/app/autenticador/uploadKey" method="post" enctype="multipart/form-data">
		<!-- <input type="hidden" name="MAX_FILE_SIZE" value="1040" /> -->
        ...
        <input id="submit" type="button" value="Autenticar" class="auth" />
    </form>
</div>
...
$('form').ajaxForm({
	complete: function(xhr) {
		if (xhr.responseText == 1) {
			...
			$('#feedback p').attr('class','error').text('Arquivo de Chave ARES maior que o esperado. Verifique sua chave.');
			$('#feedback').show().fadeOut(8000);
		} else {
			$('#feedback').hide();
			gpgValidate(xhr.responseText);
		}
	}
}); 
